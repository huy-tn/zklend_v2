...
pub fn transfer(ref self: ContractState, recipient: ContractAddress, amount: u256) -> bool {
    let amount: felt252 = amount.try_into().expect(errors::AMOUNT_OUT_OF_RANGE);
    felt_transfer(ref self, recipient, amount)
}

pub fn felt_transfer(ref self: ContractState, recipient: ContractAddress, amount: felt252) -> bool {
...
pub fn felt_approve(ref self: ContractState, spender: ContractAddress, amount: felt252) -> bool {
    let caller = get_caller_address();
...
pub fn transfer_all(ref self: ContractState, recipient: ContractAddress) -> felt252 {
    let caller = get_caller_address();
...
pub fn echo_raw_balances(ref self: ContractState, mut users: Span<ContractAddress>) {
    while let Option::Some(user) = users.pop_front() {
        self
            .emit(
...
pub fn mint(ref self: ContractState, to: ContractAddress, amount: felt252) -> bool {
...
pub fn burn(ref self: ContractState, user: ContractAddress, amount: felt252) {
    internal::only_market(@self);
...
pub fn burn_all(ref self: ContractState, user: ContractAddress) -> felt252 {
    internal::only_market(@self);
