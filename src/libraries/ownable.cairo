...
pub mod errors {
    pub const NOT_OWNER: felt252 = 'OWN_NOT_OWNER';
    pub const ZERO_ADDRESS: felt252 = 'OWN_ZERO_ADDRESS';
}
...
pub fn initializer<T, impl TOwnable: Ownable<T>, impl TDrop: Drop<T>>(
    ref self: T, owner: ContractAddress,
) {
    __private::_transfer_ownership(ref self, owner);
}

pub fn assert_only_owner<T, impl TOwnable: Ownable<T>, impl TDrop: Drop<T>>(self: @T) {
...
pub fn transfer_ownership<T, impl TOwnable: Ownable<T>, impl TDrop: Drop<T>>(
...
pub fn renounce_ownership<T, impl TOwnable: Ownable<T>, impl TDrop: Drop<T>>(ref self: T) {
    assert_only_owner(@self);
...
mod __private {
    ...
    pub fn _transfer_ownership<T, impl TOwnable: Ownable<T>, impl TDrop: Drop<T>>(
        ref self: T, new_owner: ContractAddress,
    ) {
