...
pub fn deposit(ref self: ContractState, token: ContractAddress, amount: felt252) {
    ...
    } = update_accumulators(ref self, token);

    assert_reserve_enabled(@self, token);
    let z_token_address = read_z_token_address(@self, token);
...
pub fn repay(ref self: ContractState, token: ContractAddress, amount: felt252) {
    ...
    } = repay_debt_route_internal(ref self, caller, caller, token, amount);
    self
        .emit(
...
pub fn repay_for(
    ...
    let DebtRepaid {
        raw_amount, face_amount,
    } = repay_debt_route_internal(ref self, caller, beneficiary, token, amount);
    self
        .emit(
            contract::Event::Repayment(
                contract::Repayment {
                    repayer: caller, beneficiary, token, raw_amount, face_amount,
                },
            ),
        );
}
...
pub fn repay_all(ref self: ContractState, token: ContractAddress) {
    ...
    let DebtRepaid {
        raw_amount, face_amount,
    } = repay_debt_route_internal(ref self, caller, caller, token, 0);
    self
        .emit(
            contract::Event::Repayment(
                contract::Repayment {
...
pub fn liquidate(
    ...
    let debt_reserve_decimals = read_decimals(@self, debt_token);
    let collateral_reserve = self.reserves.read(collateral_token);

    // Liquidator repays debt for user
    let DebtRepaid {
        raw_amount, ..,
    } = repay_debt_route_internal(ref self, caller, user, debt_token, amount);
...
pub fn flash_loan(
    ...
    assert_reserve_enabled(@self, token);
    let flash_loan_fee = read_flash_loan_fee(@self, token);
...
#[inline(always)]
pub fn assert_not_undercollateralized(
    self: @ContractState, user: ContractAddress, apply_borrow_factor: bool,
) {
...
pub fn is_not_undercollateralized(
    self: @ContractState, user: ContractAddress, apply_borrow_factor: bool,
) -> bool {
    ...
    } = calculate_user_collateral_data(self, user, apply_borrow_factor);
...
fn get_collateral_usd_value_required_for_token(
    ...
    if apply_borrow_factor {
        let borrow_factor = read_borrow_factor(self, token);
...
fn get_user_debt_usd_value_for_token(
    ...
    let decimals = read_decimals(self, token);
...
fn get_user_collateral_usd_value_for_token(
    ...
    // let reserve = read_for_get_user_collateral_usd_value_for_token(self,token);
    let (reserve_decimals, reserve_z_token_address, reserve_collateral_factor) =
        read_for_get_user_collateral_usd_value_for_token(
        self, token,
    );

    if reserve_collateral_factor.is_zero() {
        return 0;
    }

    // This value already reflects interests accured since last update
    let collateral_balance = IZTokenDispatcher { contract_address: reserve_z_token_address }
    ...
    let collateral_value = safe_decimal_math::mul_decimals(
        collateral_price, collateral_balance, reserve_decimals,
    );

    // Discounts value by collateral factor
    let discounted_collateral_value = safe_decimal_math::mul(
        collateral_value, reserve_collateral_factor,
    );
...
pub fn update_accumulators(ref self: ContractState, token: ContractAddress) -> UpdatedAccumulators {
    ...
    write_accumulators(
        ref self, token, block_timestamp, updated_lending_accumulator, updated_debt_accumulator,
    );
...
pub fn update_rates_and_raw_total_debt(
    ref self: ContractState,
    token: ContractAddress,
    ...
    let (interest_rate_model, raw_total_debt_before) = read_interest_rate_model_and_raw_total_debt(
        @self, token,
    );
    ...
    let ModelRates {
        lending_rate: new_lending_rate, borrowing_rate: new_borrowing_rate,
    } =
        IInterestRateModelDispatcher { contract_address: interest_rate_model }
            .get_interest_rates(reserve_balance_after, scaled_up_total_debt_after);

    // Writes to storage
    write_rates(ref self, token, new_lending_rate, new_borrowing_rate);
    if raw_total_debt_before != raw_total_debt_after {
        write_raw_total_debt(ref self, token, raw_total_debt_after);
    }
...
pub fn assert_reserve_exists(self: @ContractState, token: ContractAddress) {
    let z_token = read_z_token_address(self, token);
    assert(z_token.is_non_zero(), errors::RESERVE_NOT_FOUND);
}

/// Checks reserve is enabled.
pub fn assert_reserve_enabled(self: @ContractState, token: ContractAddress) {
    let enabled = read_enabled(self, token);
...
fn assert_debt_limit_satisfied(self: @ContractState, token: ContractAddress) {
    let debt_limit = read_debt_limit(self, token);

    let raw_total_debt = read_raw_total_debt(self, token);
...
fn assert_deposit_limit_satisfied(
    ...
    let deposit_limit = read_deposit_limit(self, token);
...
fn settle_extra_reserve_balance(ref self: ContractState, token: ContractAddress) {
    ...
    let (
        ...
        reserve_raw_total_debt,
    ) =
        read_for_settle_extra_reserve_balance(
        @self, token,
    );

    // Accumulators are already update to date from assumption
    let scaled_up_total_debt = safe_decimal_math::mul(
        reserve_raw_total_debt, reserve_debt_accumulator,
    );
    ...
    let raw_z_supply = IZTokenDispatcher { contract_address: reserve_z_token_address }
        .get_raw_total_supply();
    let owned_balance = safe_decimal_math::mul(raw_z_supply, reserve_lending_accumulator);
            ...
            .emit(
                contract::Event::AccumulatorsSync(
                    contract::AccumulatorsSync {
                        ...
                        debt_accumulator: reserve_debt_accumulator,
                    },
                ),
            );
        write_lending_accumulator(ref self, token, new_accumulator);

        // Mints fee to treasury
        if amount_to_treasury.is_non_zero() {
            IZTokenDispatcher { contract_address: reserve_z_token_address }
                .mint(treasury_addr, amount_to_treasury);
        }
    }
}
...
pub fn read_enabled(self: @ContractState, token: ContractAddress) -> bool {
    self.reserves.read(token).enabled
}

pub fn read_decimals(self: @ContractState, token: ContractAddress) -> felt252 {
    self.reserves.read(token).decimals
}

pub fn read_z_token_address(self: @ContractState, token: ContractAddress) -> ContractAddress {
    self.reserves.read(token).z_token_address
}

pub fn read_borrow_factor(self: @ContractState, token: ContractAddress) -> felt252 {
    self.reserves.read(token).borrow_factor
}

pub fn read_raw_total_debt(self: @ContractState, token: ContractAddress) -> felt252 {
    self.reserves.read(token).raw_total_debt
}

pub fn read_flash_loan_fee(self: @ContractState, token: ContractAddress) -> felt252 {
    self.reserves.read(token).flash_loan_fee
}

pub fn read_debt_limit(self: @ContractState, token: ContractAddress) -> felt252 {
    self.reserves.read(token).debt_limit
}

pub fn read_deposit_limit(self: @ContractState, token: ContractAddress) -> felt252 {
    self.reserves.read(token).deposit_limit
}

pub fn read_interest_rate_model_and_raw_total_debt(
    self: @ContractState, token: ContractAddress,
) -> (ContractAddress, felt252) {
    let reserve = self.reserves.read(token);
    (reserve.interest_rate_model, reserve.raw_total_debt)
}

pub fn read_for_get_user_collateral_usd_value_for_token(
    self: @ContractState, token: ContractAddress,
) -> (felt252, ContractAddress, felt252) {
    let reserve = self.reserves.read(token);
    (reserve.decimals, reserve.z_token_address, reserve.collateral_factor)
}

pub fn read_for_get_lending_accumulator(
    self: @ContractState, token: ContractAddress,
    ...
    (
        ...
        reserve.current_lending_rate,
    )
}

pub fn read_for_get_debt_accumulator(
    self: @ContractState, token: ContractAddress,
) -> (felt252, felt252, felt252) {
    let reserve = self.reserves.read(token);
    (reserve.last_update_timestamp, reserve.debt_accumulator, reserve.current_borrowing_rate)
}

pub fn read_for_get_pending_treasury_amount(
    self: @ContractState, token: ContractAddress,
) -> (ContractAddress, felt252, felt252, felt252, felt252) {
    let reserve = self.reserves.read(token);
    (
        reserve.z_token_address,
        reserve.reserve_factor,
        reserve.last_update_timestamp,
        reserve.lending_accumulator,
        reserve.current_lending_rate,
...
pub fn read_for_settle_extra_reserve_balance(
    self: @ContractState, token: ContractAddress,
) -> (ContractAddress, felt252, felt252, felt252, felt252) {
    let reserve = self.reserves.read(token);
    (
        ...
        reserve.raw_total_debt,
    )
}

pub fn write_lending_accumulator(
    ref self: ContractState, token: ContractAddress, lending_accumulator: felt252,
) {
    let mut reserve = self.reserves.read(token);
    reserve.lending_accumulator = lending_accumulator;
    self.reserves.write(token, reserve);
}

pub fn write_raw_total_debt(
    ref self: ContractState, token: ContractAddress, raw_total_debt: felt252,
) {
    let mut reserve = self.reserves.read(token);
...
pub fn write_interest_rate_model(
...
pub fn write_collateral_factor(
...
pub fn write_borrow_factor(
    ref self: ContractState, token: ContractAddress, borrow_factor: felt252,
) {
    let mut reserve = self.reserves.read(token);
    reserve.borrow_factor = borrow_factor;
    self.reserves.write(token, reserve);
}

pub fn write_reserve_factor(
...
pub fn write_debt_limit(ref self: ContractState, token: ContractAddress, debt_limit: felt252) {
    let mut reserve = self.reserves.read(token);
    reserve.debt_limit = debt_limit;
    self.reserves.write(token, reserve);
}

pub fn write_deposit_limit(
    ref self: ContractState, token: ContractAddress, deposit_limit: felt252,
) {
    let mut reserve = self.reserves.read(token);
    reserve.deposit_limit = deposit_limit;
    self.reserves.write(token, reserve);
}
...
pub fn write_accumulators(
    ...
    reserve.lending_accumulator = lending_accumulator;
    reserve.debt_accumulator = debt_accumulator;
    self.reserves.write(token, reserve);
}
...
pub fn write_rates(
    ...
    let mut reserve = self.reserves.read(token);
    reserve.current_lending_rate = current_lending_rate;
    reserve.current_borrowing_rate = current_borrowing_rate;
    self.reserves.write(token, reserve);
}
